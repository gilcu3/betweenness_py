...
import datasets.toobig.facebook.load as facebook

def measure(params):
    now = time()
    mem = memory_usage(params, max_usage=True)[0]
    return time() - now, mem


def pmeasure(A0, edge, undirected_directed, filestr):
    if undirected_directed:
        res0 = measure((A0.update, (edge,)))
        res1 = measure((A0.update, ((edge[1], edge[0]),)))
...
def full(algo, dataset, edges):
    ...
    filestr = 'test/%s.%s.out' % (algo.__module__, dname)
...
if __name__ == '__main__':
    ...
    filestr = 'edges/%s.bin' % name

    if not os.path.exists(filestr):
        edges = [data.gen_new_edge(g) for _ in range(100)]
    ...
    # for algo in [toy.algo, ibet.algo, brandes.algo, icentral.algo]:
    full(algo, d, edges)
    print('time', time() - now)
