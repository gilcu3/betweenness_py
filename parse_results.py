...
def parse():
    ...
    def process_plot(datas, filename):
        with open(path_out + filename + '.plot', 'w') as fresults:
            line = ['D']
            line += ['Brandes_time', 'Brandes_mem', 'Ibet_time', 'Ibet_mem', 'Bidcentral_time', 'Bidcentral_mem']
            print(' '.join(line), file=fresults)
    ...
    process_tab(real_u, 'results_add_real_u')
    process_tab(real_d, 'results_add_real_d')
    process_tab(sin_d, 'results_add_sin_d')
    process_plot(sin_d, 'results_add_sin_d')
