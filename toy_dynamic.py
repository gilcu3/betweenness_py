...
def _bidcentral(g: nx.DiGraph, bc, e, op):
    nbc = bc.copy()
    u, v = e
    ng = g.copy()
    n = g.number_of_nodes()
...
class algo():
    ...
    def update(self, edge):
        nbc = _bidcentral(self.g, self.bc, edge, ops.ADD)
        self.g.add_edge(edge[0], edge[1])
        return nbc

    def remove(self, edge):
        # print(edge, edge in self.g.edges)
        nbc = _bidcentral(self.g, self.bc, edge, ops.DELETE)
...
def main():
    ...
    g = nx.DiGraph([(2, 1), (1, 0)])
    # nx.nx_agraph.view_pygraphviz(g)
    bc = nx.betweenness_centrality(g, normalized=False)
    print(bc)
    edge = (1, 0)
    nbc = _bidcentral(g, bc, edge, ops.DELETE)
